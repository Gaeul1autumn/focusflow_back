...
public interface DailyStatisticRepository extends MongoRepository<DailyStatistic, String> {
    ...
    Optional<DailyStatistic> findByUserIdAndDate(String userId, LocalDate date);

    // (나중을 위해) 특정 유저의 기간별 조회 (예: 최근 7일)
    List<DailyStatistic> findByUserIdAndDateBetweenOrderByDateAsc(String userId, LocalDate startDate, LocalDate endDate);
}
